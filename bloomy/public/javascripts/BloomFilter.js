...
BloomFilter.prototype = {
    ...
    init: function(){
        ...
        */
        var ret = this._calculate_shortest_filter_length(this.capacity, this.error_rate);
        var length = ret[0];
        var num_funcs = ret[1];
        this.num_hash_funcs = num_funcs;
        this.filter_length = length;
    
        // # create some random salts;
        var seen = {};
        var salts = [];
        while(salts.length < this.num_hash_funcs){
            var rand = Math.random();
            if(seen[rand]) continue;
            salts.push(rand);
        }
        this.salts = salts;

        // # make an empty filter
        this.filter = this._make_empty_filter(this.filter_length);

        // # make some blank vectors to use
        // this.blankvec = new Array(4).fill(false);
        return 1;
    },
    on_bits: function(){
        if (!this.filter) return;
        return this.filter.filter(function(bit){return bit}).length;
    },
    add: function(arg){
        var keys = (arg instanceof Array) ? arg : arguments;
        for(var i=0;i<keys.length; i++){
            if(this.key_count > this.capacity){
                // error
                return;
            }
            var key = keys[i];
            // # flip the appropriate bits on
            var cells = this._get_cells(key);
            for(var j=0;j<cells.length;j++)
                this.filter[cells[j]] = BloomFilter.TRUE; // true
            this.key_count++;
        }
        return this;
    },
    check: function(arg){
        ...
        for(var i=0;i<keys.length;i++){
            ...
            for(var j = 0; j < cells.length; j++){
                var cell = cells[j];
                // match = check_bit_on(this.filter, cell);
                match = this._check_bit_on(cell);
                if (!match) break;
            }
            result.push(match);
        }
    ...
    _calculate_shortest_filter_length: function(num_keys, error_rate){
        var lowest_m;
        var best_k = 1;
        for(var k=1;k<=100;k++){
            var m = (-1 * k * num_keys) / ( Math.log( 1 - Math.pow(error_rate, (1/k)) ));
    ...
    _get_cells: function(key){
        ...
        for(var i=0; i < salts.length; i++){
            ...
            var result = vec;
            // console.log(result);
            var bit_offset = Math.abs(result  % this.filter_length);
            cells.push(bit_offset)
        }
        return cells;
    },
    _check_bit_on: function(offset){
        return this.filter[offset] == BloomFilter.TRUE;
    },
    _make_empty_filter: function(size){
        var f = new Array(size);
        for(var i=0;i<size;i++) f[i] = BloomFilter.FALSE ; // false
        return f;
    }
};
...
function sha1(){
    ...
    var str = r.join("");
    var sha1_hex = CybozuLabs.SHA1.calc(str);
    return sha1_hex;
}
...
new function() {
    CybozuLabs.SHA1.calc = CybozuLabs.SHA1.calc_std;
};
